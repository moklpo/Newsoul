...
COEFF = 1.0
AP = 14
CHANGE_FILTER = 1.012  # +1.2% from Open for Buy
SELL_FILTER = 0.988    # -1.2% from Open for Sell
...
# --- TOP MOVERS FILTER (Momentum from Today's Open) ---
def get_top_movers(fyers, all_stocks):
    movers = []
    print(f"🔍 Filtering Intraday Momentum (vs Today's Open)...")
    batch_size = 50
    ...
    print(f"✅ Selected {len(final_list)} stocks with strongest momentum from Open.")
    return final_list
    ...
    for s in active_stocks:
        try:
            ...
            if signal_type and (s not in notified_stocks or time.time() - notified_stocks[s] > 300):
                ...
                msg = (f"{signal_type}: *{s}*\n"
                       f"Price: {curr['close']}\n"
                       f"Move from Open: {((curr['close']-day_open)/day_open)*100:.2f}%")
                send_telegram(msg)
            time.sleep(0.01)
        except: continue
...
# Start Message
start_msg = "🚀 *AlphaTrend Momentum Bot Online*\n━━━━━━━━━━━━━━━━━━\n✅ Login: Success\n🎯 Tracking: Top 20 Gainers/Losers (from Open)\n📈 Filter: 1.2% Intraday Move"
...
print("🚀 Bot Started: Filtering by Intraday Momentum")
while True:
