...
if token:
    ...
    requests.post(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage", 
                  json={"chat_id": TELEGRAM_CHAT_ID, "text": "🚀 *Bot Online:* Scanning with +1.2%/-1.2% Directional Filter...", "parse_mode": "Markdown"})
else:
    ...
    for s in stocks:
        try:
            ...
            if h5['s'] == 'ok' and hd['s'] == 'ok':
                ...
                # Pivot Calc - Complete Classic Pivot Set (P, R1-R3, S1-S3)
                prev = dfd.iloc[-2]
                ...
                if rng > 0 and (body/rng) >= 0.6:
                    # ✅ BUY: If Stock is Up > 1.2% AND Candle is Bullish AND level cross
                    if change_pct >= 1.2 and c_close > c_open:
                    ...
                    # ✅ SELL: If Stock is Down < -1.2% AND Candle is Bearish AND level cross
                    elif change_pct <= -1.2 and c_close < c_open:
                        for name, val in levels.items():
